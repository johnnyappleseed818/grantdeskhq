...
export async function executeFinalInstantlyHandoff(input: { email: string; campaignId: string; subject: string; body: string; sequenceId: string; source: string }, dependencies: FinalHandoffDependencies) {
  const validated = validateInstantlyOutboundInput(input);
  ...
  try {
    ...
    if (!externalLeadId) await dependencies.fail(idempotencyKey, validated.email, error instanceof Error ? error.message : "Instantly handoff failed.");
    throw error;
  }
}
