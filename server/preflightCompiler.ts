...
Keep the source status of a contractual rule separate from whether current compliance has been calculated. An explicitly stated indirect-cost cap is source-supported even when current spending still needs validation. Extract emergency-assistance documentation for every assistance transaction separately from any additional approval required above a per-transaction threshold.
