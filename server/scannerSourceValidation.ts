...
const approvedScannerFolderHosts = new Set(["linkedin.com", "indeed.com", "idealist.org", "lensa.com", "mcjobboard.net"]);
...
export async function validateScannerSourceSeeds(env: NodeJS.ProcessEnv = process.env) {
  ...
  const result = { selected: candidates.length, validated: 0, roleUnresolved: 0, rejected: 0, provider: "scrapegraph", creditsRemaining: null as number | null, blocked: null as string | null };
  ...
  for (const seed of candidates) {
    ...
    if (!source || approvedScannerFolderHosts.has(source.hostname)) {
    ...
    const organizationDomain = source.hostname.replace(/^www\./, "");
