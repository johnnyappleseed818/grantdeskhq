...
async function handleControlledInstantlyBatch(request: IncomingMessage, response: ServerResponse) {
  ...
  const preflightTelemetry = { batchId, execute: input.execute === true, mailboxReady, apiClientReady: Boolean(client), integrationEnabled: config.integrationEnabled, apiKeyConfigured: config.apiKeyConfigured, controlledBatchEnabled: config.controlledBatchEnabled, controlledBatchMatches: config.controlledBatchId === batchId, campaignConfigurationAllowed, selectedCampaigns: selectedCampaigns.length, directEligible: direct.length, partnerEligible: partner.length, directCampaignLeads: campaignLeadCount.get(config.directCampaignId) || 0, partnerCampaignLeads: campaignLeadCount.get(config.partnerCampaignId) || 0 };
  console.info("GTM_CONTROLLED_BATCH_PREFLIGHT " + JSON.stringify(preflightTelemetry));
  ...
  if (!mailboxReady || !client || !campaignConfigurationAllowed) return json(response, 409, { error: "Approved mailbox or mapped campaigns are not safe for a controlled batch.", gate: preflightTelemetry });
