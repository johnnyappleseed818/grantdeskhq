...
async function handleGtmChannelSeedEnrich(request: IncomingMessage, response: ServerResponse) {
  ...
  const provider = result.providerStatus === "HUNTER_COMPLETED" ? "hunter" : result.providerStatus === "SCRAPEGRAPH_INSTANTLY_VERIFICATION" ? "scrapegraphai+instantly_email_verification" : "instantly_supersearch";
  console.info(JSON.stringify({ event: "GTM_CHANNEL_SEED_ENRICHMENT", provider, segment: result.segment, selected: result.selected, previewCount: result.previewCount, verifiedContacts: provider === "hunter" || provider.startsWith("scrapegraphai") ? result.submitted : null, pendingVerification: "pendingVerification" in result ? result.pendingVerification : null, noPublishedContact: "noPublishedContact" in result ? result.noPublishedContact : null, submitted: result.submitted, resourceIdPresent: Boolean(result.resourceId), blocked: result.blocked, timestamp: new Date().toISOString() }));
...
async function handleAmbiguousProviderOutcomeResolution(request: IncomingMessage, response: ServerResponse) {
  ...
  const legacyTombstonedMembership = Boolean(
    providerLead
    && reservation
    && !providerSameCampaign
    && [config.legacyDirectCampaignId, config.legacyPartnerCampaignId].includes(providerCampaignId)
    && tombstone
  );
  const providerCrossCampaignConflict = Boolean(providerLead && reservation && !providerSameCampaign && !legacyTombstonedMembership);
  ...
  const prerequisites = ambiguousProviderOutcomePrerequisites({
    ...
    canonicalOrTombstoneIdentityPresent: Boolean((canonical && reservation && canonical.email && canonical.organizationId) || legacyTombstonedMembership),
  ...
  const providerResult = providerCrossCampaignConflict ? "CROSS_CAMPAIGN_CONFLICT" : legacyTombstonedMembership ? "LEGACY_TOMBSTONED_MEMBERSHIP" : providerSameCampaign ? "ADOPT_EXISTING_MEMBERSHIP" : reservation ? "NO_PROVIDER_MEMBERSHIP" : "NOT_EVALUATED";
  try {
    console.info(JSON.stringify({ event: "OUTBOUND_AMBIGUOUS_PROVIDER_RESOLUTION", mode, eventId: outboundCircuitEventId(circuit), version: circuit.version, prerequisites: { ...prerequisites, allRequiredFlags }, campaignStates: { direct: Number(directCampaign.status), partner: Number(partnerCampaign.status) }, unresolvedReservationCount: selection.unresolved.length, selection: selection.reason, providerResult, resolutionRef, timestamp: new Date().toISOString() }));
  ...
  if (!apply) return json(response, 200, {
    ...
    providerResult,
    plannedAction: providerSameCampaign ? "ADOPT_EXISTING_MEMBERSHIP_AND_CLOSE" : legacyTombstonedMembership ? "RETAIN_LEGACY_TOMBSTONE_AND_CLOSE" : "QUARANTINE_UNRESOLVED_IDENTITY_AND_CLOSE",
    resolutionRef
  });
  ...
  if (providerSameCampaign) {
    ...
    await reconcileInstantlyPolling();
  } else if (legacyTombstonedMembership && tombstone) {
    // The legacy provider membership and immutable tombstone already prove the
    // contact is permanently excluded.  Never "repair" it by changing either
    // provider membership or historical canonical evidence.
    resolutionRecordId = tombstone.tombstoneId;
  } else {
...
function controlledEmail(record: Awaited<ReturnType<typeof readCanonicalGtmModel>>["records"][number]) {
  ...
  const close = record.segment === "DIRECT"
    ? "We're offering introductory Growth pricing to 25 nonprofit customers at $99/month, normally $199/month. Would you be open to trying it with one award for free? https://grantdeskhq.com/assessment"
    : "If you'd like to see whether it is useful for a client, you can try one real client award for free: https://grantdeskhq.com/assessment";
...
function controlledCampaignPatch(segment: "DIRECT" | "PARTNER", maximum: number, sender: string) {
  ...
  const initialBody = direct
    ? "<div>Hi {{firstName}},</div><div><br /></div><div>{{openingLine}}</div><div><br /></div><div>Preparing a funder report can mean pulling together award terms, accounting data, program updates, and supporting evidence from several places.</div><div><br /></div><div>GrantDeskHQ turns those inputs into a source-linked first draft for human review. People remain responsible for the final review and submission.</div><div><br /></div><div>We're offering introductory Growth pricing to 25 nonprofit customers at $99/month, normally $199/month. Would you be open to trying it with one award for free? https://grantdeskhq.com/assessment</div><div><br /></div><div>Best,</div><div>Eli</div>"
...
/** Scheduler inputs may request only automatic evaluation. This is the sole
 * autonomous dispatch controller; every send decision is derived from canonical
 * state and current provider evidence, never from execute/limit payloads. */
async function handleAutomaticInstantlyDispatch(request: IncomingMessage, response: ServerResponse) {
  if (request.method !== "POST") return json(response, 405, { error: "Method not allowed." });
  await requireGtmScheduler(request);
  const input = await readJson(request) as { mode?: unknown; segment?: unknown };
  const segment = input.segment === "DIRECT" || input.segment === "PARTNER" ? input.segment as DispatchSegment : null;
  if (input.mode !== "auto" || !segment) return json(response, 400, { error: "Dispatch requires mode:auto and a valid segment." });
  await reconcileInstantlyPolling();
  const config = instantlyConfig();
