...
async function handleControlledInstantlyBatch(request: IncomingMessage, response: ServerResponse) {
  ...
  const gate = !mailboxReady ? "MAILBOX" : !client ? "API_CLIENT" : !campaignConfigurationAllowed ? "CAMPAIGN_CONFIGURATION" : !config.controlledBatchEnabled || config.controlledBatchId !== batchId ? "BATCH_CONFIGURATION" : (direct.length + partner.length === 0) ? "NO_ELIGIBLE_CONTACTS" : "READY";
  const reasonCode = gate === "MAILBOX" ? "SENDER_NOT_READY" : gate === "API_CLIENT" ? "INTEGRATION_OR_KEY_UNAVAILABLE" : gate === "CAMPAIGN_CONFIGURATION" ? "MAPPED_CAMPAIGN_UNSAFE" : gate === "BATCH_CONFIGURATION" ? "EXACT_BATCH_DISABLED" : gate === "NO_ELIGIBLE_CONTACTS" ? "CANONICAL_READY_EMPTY" : "OK";
  ...
  console.info(preflightTelemetry);
