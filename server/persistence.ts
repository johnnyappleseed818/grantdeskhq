...
export async function saveGtmChannelSeed(seed: ChannelSeedRecord): Promise<ChannelSeedRecord> {
  ...
  await writeDocument(accessToken, `gtm/channel-seeds/records/${safeChannelSeedDocumentId(seed.id)}`, {
    ...
    enrichmentResourceId: seed.enrichmentResourceId || "", enrichmentOperationId: seed.enrichmentOperationId || "", enrichmentBackgroundJobId: seed.enrichmentBackgroundJobId || "", enrichmentJobId: seed.enrichmentJobId || "", enrichmentProviderStatus: seed.enrichmentProviderStatus || "", enrichmentSubmittedAt: seed.enrichmentSubmittedAt || "", enrichmentLastCheckedAt: seed.enrichmentLastCheckedAt || "", enrichmentAttemptCount: seed.enrichmentAttemptCount || 0, enrichmentLastProviderError: seed.enrichmentLastProviderError || "", enrichmentTerminalAt: seed.enrichmentTerminalAt || "", enrichmentUpdatedAt: seed.enrichmentUpdatedAt || "",
...
export interface GtmDispatchActivation {
  ...
  outcome: "NONE" | "ACCEPTED" | "SENT" | "FAILED";
  dailyCapacity: number;
  lastSuccessfulDispatchAt: string;
  failureReason: string;
  stateVersion: number;
  updatedAt: string;
}
function dispatchActivationPath(segment: "DIRECT" | "PARTNER") { return `gtm/instantly/dispatch-activations/${segment.toLowerCase()}`; }
export async function readGtmDispatchActivation(segment: "DIRECT" | "PARTNER"): Promise<GtmDispatchActivation | null> {
  const token = await gcpToken();
  const response = await authorizedFetch(`${firestoreBase}/${dispatchActivationPath(segment)}`, token);
  if (response.status === 404) return null;
  if (!response.ok) throw new Error(`Dispatch activation could not be loaded (${response.status}).`);
