...
## 2026-09-23 06:16–06:37 UTC: scheduled new-source routing repair

- The first remaining upstream defect was reproducible from the durable job configuration and the prior scheduled run: the job named `grantdeskhq-daily-social-scan` does run the full Direct discovery worker, and `grantdeskhq-daily-partner-reconciliation` runs Partner discovery, but both routed newly discovered organizations into the legacy Hunter batch. The already configured canonical channel-seed → Instantly SuperSearch/verification path was not used for those fresh records. The independent Drive validation worker correctly selected zero records because all current scanner records were terminal or not due; that is data state, not a scheduler failure.
- Commit `54af5b297c538c9e47dbbad090f1873080e2e02e` changes only scheduled new Direct and Partner organization handling: after idempotent canonical seed import, it submits the same `ENRICHMENT_PENDING` seed route used by scanner imports to Instantly SuperSearch and reconciles it. Historical Hunter-backed records retain read-only reconciliation; no new scheduled discovery work requires Hunter.
- Focused route/safety suite: 63 tests passed; TypeScript, lint, production build, and diff checks passed. Cloud Build `417d94df-08e1-4d38-a8dd-1b81960ff68b` succeeded. Zero-traffic candidate `grantdeskhq-prototype-00547-gum` passed health, unauthenticated importer rejection (401), and scheduler-authenticated read-only reconciliation with the existing Clean campaign mapping and required flags preserved.
- `grantdeskhq-prototype-00547-gum` now serves 100% of production traffic; the custom-domain health endpoint returned HTTP 200. No manual source validation, SuperSearch call, campaign activation, enrollment, or email was performed in this deployment. The next durable source execution is Drive import at 08:15 America/Detroit, then scanner validation at 08:30, Direct enrichment at 08:45, Partner enrichment at 08:50, and the full Direct discovery worker at 09:35. Those future jobs are the remaining proof of fresh-record business action.
- The production scheduler-authenticated sourcing status at `2026-09-23T11:00:29Z` shows the precise prior-run bottleneck, not a zero-data inference: the `2026-09-22T13:35:21Z` Direct discovery scan examined 6 organizations, accepted 5 new organizations, and left zero READY because its legacy email-verification branch recorded a provider error. Current Direct canonical state is 0 READY, 4 NEEDS_VERIFICATION, 124 RESEARCH_BACKLOG, 14 AWAITING_REPLY, and 2 ALREADY_CONTACTED. The now-deployed channel-seed enrichment worker is scheduled before the next daily discovery run, so it will process safely actionable existing seeds without manual intervention.
