...
- Read-only live evidence before deployment showed `/qa-nonexistent-path` returning the old HTTP 200 SPA shell. The candidate static-serving branch now selects `404.html` at HTTP 404 in Cloud Run, and the regression test pins that branch. A direct local Cloud Run process could not be launched because the workspace Node 24 runtime rejects an unrelated extensionless TypeScript ESM import; TypeScript compilation passed and the production container uses Node 22. No deployment or traffic change was made.
