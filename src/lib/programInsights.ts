...
export function buildRetentionInsight(result: Pick<CompilationResult, "narrative" | "requirements">): ProgramInsight | null {
  ...
  const retentionRequirement = result.requirements.find((item) => item.status === "verified" && /retention|remain(?:ed)?\s+(?:stably\s+)?housed/i.test(item.requirement) && /%/.test(item.requirement));
