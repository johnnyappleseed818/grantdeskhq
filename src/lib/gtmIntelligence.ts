export type SignalType = "RECENT_GRANT" | "GRANT_HIRING" | "GRANT_ACCOUNTING_HIRING" | "GRANTS_MANAGER_HIRING" | "REPORTING_PAIN" | "FINANCE_PAIN" | "SPREADSHEET_PAIN" | "RECONCILIATION_PAIN" | "PARTNER_SIGNAL" | "OTHER";
export interface BuyerLanguageObservation { phrase: string; sourceType: string; segment: string; painCategory: string; buyerStage: string; relevance: number; observedAt: string; sourceUrl: string; }
export interface BuyerLanguageTheme extends BuyerLanguageObservation { evidenceCount: number; lastObserved: string; }
export interface ProspectScoreInput { fit: number; intent: number; timing: number; contactability: number; procurementFriction: number; }
export interface ProspectScore { total: number; factors: ProspectScoreInput; explanation: string[]; }
export type PartnerRelationship = "A" | "B" | "C" | "D";
export interface FollowUpRecord { id: string; dueAt: string; suppressed: boolean; status: "DRAFT_READY" | "APPROVED" | "SENT" | "REPLIED" | "CLOSED"; }
export const clamp = (value: number) => Math.max(0, Math.min(10, Math.round(value)));
