...
export function GtmDashboardContent({ dailySignalToken, initialDailyScan = null, initialAwardScan = null, initialControlPlane = null, initialOverview = null, seedOpportunities = [] }: { dailySignalToken?: GtmTokenProvider; initialDailyScan?: DailySocialScan | null; initialAwardScan?: AwardDiscoveryScan | null; initialControlPlane?: ControlPlaneQueueReconciliation | null; initialOverview?: GtmOverview | null; seedOpportunities?: GtmOpportunity[] } = {}) {
  ...
  useEffect(() => {
    if (!dailySignalToken) return;
    let active = true;
    requestGtmWithFreshToken<{ health: InstantlyHealth; persisted: InstantlyPersistedStatus | null }>(dailySignalToken, "/api/gtm/instantly")
      .then((body) => { if (active) setInstantly(body); })
  ...
  const canonicalOutreachMetrics = useMemo(() => summarizeCanonicalProviderOutreach(canonical), [canonical]);
  ...
  return <div className="gtm-page">
    <header className="gtm-header">
      <div className="site-shell py-9 lg:py-12">
        ...
        <div className="gtm-metrics" aria-label="Commercial KPIs">
          ...
          <Metric icon={MailCheck} label="Unique contacted" value={canonical ? canonicalOutreachMetrics.uniqueOrganizationsContacted : outreachMetrics.uniqueOrganizationsContacted} detail={canonical ? canonicalOutreachMetrics.directUniqueOrganizationsContacted + " direct · " + canonicalOutreachMetrics.partnerUniqueOrganizationsContacted + " partner" : outreachMetrics.directUniqueOrganizationsContacted + " direct · " + outreachMetrics.partnerUniqueOrganizationsContacted + " partner"} />
    ...
    <div className="site-shell py-8 lg:py-12">
      ...
      {activeTab === "opportunities" && canonicalRuntime && (canonicalLoading ? <section className="workspace-empty"><LoaderCircle className="animate-spin" aria-hidden="true" /><h2>Loading canonical GTM records</h2></section> : canonicalError ? <section className="workspace-empty"><AlertCircle aria-hidden="true" /><h2>Unable to load GTM records.</h2><p>{canonicalError}</p><button type="button" className="button button-primary" onClick={() => setCanonicalRetry((value) => value + 1)}>Retry</button></section> : <GtmOpportunityQueue model={canonical} />)}
      ...
      {activeTab === "outreach" && (canonicalRuntime ? <ProviderOutreachHistoryPanel model={canonical} /> : <OutreachHistoryPanel records={outreach} canonicalOpportunityIds={controlPlane?.cards.map((card) => card.canonicalCardId) || ranked.map((opportunity) => opportunity.id)} filter={outreachFilter} query={outreachQuery} onFilter={setOutreachFilter} onQuery={setOutreachQuery} />)}
      ...
      {activeTab === "system-health" && <>{canonicalRuntime ? <GtmExecutiveOverview model={canonical} health={instantly?.health || null} persisted={instantly?.persisted || null} seo={seoSummary(searchConsole, contentEngine)} /> : <OverviewPanel overview={overview} inMain />}<div className="mt-8"><InstantlyHealthPanel health={instantly?.health || null} persisted={instantly?.persisted || null} /></div><div className="mt-8"><SignalsPanel dailyScan={dailyScan} directDiscovery={directDiscovery} loading={signalsLoading} error={signalsError} /></div><div className="mt-8"><SourcesPanel dailyScan={dailyScan} directDiscovery={directDiscovery} /></div><div className="mt-8"><AccuracyPanel /></div></>}
    </div>
  </div>;
}
...
function SeoQueuePanel({ state, content, error, pendingId, onUpdate }: { state: SearchConsoleState | null; content: ContentEngineState | null; error: string; pendingId: string | null; onUpdate(kind: "opportunity" | "draft" | "distribution", id: string, status: string): Promise<void> }) {
  ...
  const draftsByOpportunity = new Map((content?.drafts || []).map((draft) => [draft.opportunityId, draft]));
...
function summarizeCanonicalProviderOutreach(model: CanonicalGtmModel | null) {
  const sent = (model?.records || []).filter((record) => Boolean(record.sentAt) && ["SENT", "REPLIED", "POSITIVE", "BOUNCED", "UNSUBSCRIBED", "NOT_INTERESTED", "SEQUENCE_COMPLETE"].includes(String(record.instantlyStatus || "").toUpperCase()));
  const organizations = (segment?: "DIRECT" | "PARTNER") => new Set(sent.filter((record) => !segment || record.segment === segment).map((record) => record.organizationId)).size;
  return { uniqueOrganizationsContacted: organizations(), directUniqueOrganizationsContacted: organizations("DIRECT"), partnerUniqueOrganizationsContacted: organizations("PARTNER"), sent };
}

function ProviderOutreachHistoryPanel({ model }: { model: CanonicalGtmModel | null }) {
  const summary = summarizeCanonicalProviderOutreach(model);
  return <section aria-labelledby="provider-outreach-history-heading"><div className="gtm-section-heading"><div><p className="eyebrow">Provider-confirmed outreach history</p><h2 id="provider-outreach-history-heading">Instantly events</h2><p>Only persisted provider events appear here. Enrollment and scheduled status are not email sends.</p></div><span className="status-badge status-neutral">PROVIDER SOURCE</span></div><div className="gtm-automation-metrics"><article><strong>{summary.sent.length}</strong><span>provider-confirmed send events</span></article><article><strong>{summary.uniqueOrganizationsContacted}</strong><span>unique organizations contacted</span></article><article><strong>{summary.directUniqueOrganizationsContacted}</strong><span>direct unique</span></article><article><strong>{summary.partnerUniqueOrganizationsContacted}</strong><span>partner unique</span></article></div><div className="panel panel-flush mt-6"><div className="table-scroll"><table className="data-table gtm-pipeline-table"><thead><tr><th>Organization</th><th>Contact</th><th>Segment</th><th>Provider status</th><th>First sent</th><th>Reply</th><th>Bounce / unsubscribe</th><th>Next action</th></tr></thead><tbody>{summary.sent.map((record) => <tr key={record.id}><th>{record.organization}</th><td>{record.contact || "Not recorded"}</td><td>{record.segment === "DIRECT" ? "Direct" : "Partner"}</td><td>{record.instantlyStatus || "Not recorded"}</td><td>{formatHistoryDate(record.sentAt || null)}</td><td>{record.state === "REPLIED" || record.state === "POSITIVE" ? "Recorded" : "—"}</td><td>{["BOUNCED", "UNSUBSCRIBED"].includes(String(record.instantlyStatus || "").toUpperCase()) ? String(record.instantlyStatus) : "—"}</td><td>{record.nextAction}</td></tr>)}{!summary.sent.length && <tr><td colSpan={8}>No provider-confirmed prospecting sends are recorded.</td></tr>}</tbody></table></div></div></section>;
}
