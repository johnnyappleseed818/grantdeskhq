    ...
    <header className="gtm-header">
      <div className="site-shell py-9 lg:py-12">
        ...
        <div className="gtm-metrics" aria-label="Commercial KPIs">
          <Metric icon={MailCheck} label="Sent" value={outreachMetrics.totalSent} detail={outreachMetrics.directSent + " direct · " + outreachMetrics.partnerSent + " partner"} />
...
function FunnelCard({ title, records }: { title: string; records: OutreachRecord[] }) { const direct = title === "Direct funnel"; const stages: Array<[string, string | number]> = direct ? [["Qualified", "Not instrumented"], ["Ready to send", "Not instrumented"], ["Sent", records.length], ["Replies", records.filter((record) => record.replied).length], ["Positive replies", records.filter((record) => record.replySentiment === "POSITIVE").length], ["Free First Award", records.filter((record) => record.trial).length], ["Award uploaded", "Not instrumented"], ["Report generated", "Not instrumented"], ["Paid", records.filter((record) => record.customer).length]] : [["High fit", "Not instrumented"], ["Ready to send", "Not instrumented"], ["Sent", records.length], ["Replies", records.filter((record) => record.replied).length], ["Positive replies", records.filter((record) => record.replySentiment === "POSITIVE").length], ["Active conversations", "Not instrumented"], ["Trial with client or award", records.filter((record) => record.trial).length], ["Active partners", "Not instrumented"], ["Customers influenced", "Not instrumented"], ["Paid customers influenced", records.filter((record) => record.customer).length]]; return <div className="panel"><p className="eyebrow">{title}</p><div className="gtm-automation-metrics mt-4" aria-label={title}>{stages.map(([label, value]) => <article key={label}><strong>{value}</strong><span>{label}</span></article>)}</div></div>; }
...
function PartnersPanel({ records: _records, overview: _overview }: { records: OutreachRecord[]; overview: GtmOverview | null }) {
