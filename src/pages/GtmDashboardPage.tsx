...
export function GtmDashboardContent({ dailySignalToken, initialDailyScan = null, initialAwardScan = null, initialControlPlane = null, initialOverview = null, seedOpportunities = [] }: { dailySignalToken?: () => Promise<string>; initialDailyScan?: DailySocialScan | null; initialAwardScan?: AwardDiscoveryScan | null; initialControlPlane?: ControlPlaneQueueReconciliation | null; initialOverview?: GtmOverview | null; seedOpportunities?: GtmOpportunity[] } = {}) {
  ...
  return <div className="gtm-page">
    ...
    <div className="site-shell py-8 lg:py-12">
      {activeTab === "overview" && <FounderOverview records={scheduledOutreach} overview={overview} sendReadyCustomers={sendReadyCustomers} needsVerificationCustomers={needsVerificationCustomers} onOpenOutreach={() => setActiveTab("customers")} />}
      ...
      {activeTab === "customers" && <><CustomerSendReadyPanel opportunities={sendReadyCustomers} needsVerification={needsVerificationCustomers} onReview={(id) => setExpanded(id)} /><OutreachHistoryPanel records={scheduledOutreach.filter((record) => record.type === "DIRECT_NONPROFIT")} canonicalOpportunityIds={controlPlane?.cards.map((card) => card.canonicalCardId) || ranked.map((opportunity) => opportunity.id)} filter={outreachFilter} query={outreachQuery} onFilter={setOutreachFilter} onQuery={setOutreachQuery} /></>}
      {activeTab === "social" && <SocialEngagementPanel />}
      {activeTab === "partners" && <PartnersPanel records={scheduledOutreach} overview={overview} />}
...
function FounderOverview({ records, overview, sendReadyCustomers, needsVerificationCustomers, onOpenOutreach }: { records: OutreachRecord[]; overview: GtmOverview | null; sendReadyCustomers: GtmOpportunity[]; needsVerificationCustomers: number; onOpenOutreach(): void }) {
...
function SocialEngagementPanel() {
  const reddit = redditSignals as Array<{ id: string; title: string; url: string; community: string; evidenceSummary: string; productImplication: string; observedAt: string; }>;
  const linkedin = linkedinItems as Array<{ title: string; url: string; observedPain: string; suggestedComment?: string; status: string; author?: string; }>;
  return <section aria-labelledby="social-engagement-heading"><div className="gtm-section-heading"><div><p className="eyebrow">Manual engagement queue</p><h2 id="social-engagement-heading">Review public conversations before engaging</h2><p>No posting, voting, direct messages, or automatic engagement occurs here. Source dates are shown only when recorded.</p></div><span className="status-badge status-neutral">MANUAL REVIEW</span></div><div className="grid gap-6 lg:grid-cols-2"><div className="panel"><div className="panel-heading"><div><p className="eyebrow">Reddit</p><h3>{reddit.length} researched threads</h3></div></div><div className="gtm-feed-list">{reddit.map((item) => <article key={item.id}><MessageSquareText aria-hidden="true" /><div><a href={item.url} target="_blank" rel="noreferrer">{item.title}<ExternalLink aria-hidden="true" /></a><p>{item.evidenceSummary}</p><small>{item.community} · Observed {formatDate(item.observedAt)}; recheck post age before engagement.</small><p><strong>Suggested helpful angle:</strong> {item.productImplication}</p><span className="status-badge status-review">NEW · REVIEW BEFORE RESPONSE</span></div></article>)}</div></div><div className="panel"><div className="panel-heading"><div><p className="eyebrow">LinkedIn</p><h3>Public opportunities and group review</h3></div></div><div className="gtm-feed-list">{linkedin.map((item) => <article key={item.url}><UsersRound aria-hidden="true" /><div><a href={item.url} target="_blank" rel="noreferrer">{item.title}<ExternalLink aria-hidden="true" /></a><p>{item.observedPain}</p><small>{item.author || "Author not recorded"} · {item.status.replaceAll("_", " ")}</small>{item.suggestedComment && <p><strong>Suggested helpful comment:</strong> {item.suggestedComment}</p>}<span className="status-badge status-review">REVIEW BEFORE RESPONSE</span></div></article>)}</div><p className="mt-4 text-sm text-slate-600">No private LinkedIn group content has been verified. For a private group, paste the post URL and text into the approved manual-review workflow; do not infer access or activity.</p></div></div></section>;
}
