...
export function PilotPage() {
  ...
  const submit = (event: FormEvent) => {
    ...
    const body = `Hi Eli,\n\nI would like to discuss the GrantDesk Founding Agency Pilot.\n\nName: ${form.name}\nWork email: ${form.email}\nFirm: ${form.firm}\nRole: ${form.role}\nApproximate nonprofit clients: ${form.clients}\nCurrent reporting process: ${form.process}\n\nI understand this website is an interactive prototype using synthetic data and that no files should be submitted through this enquiry.`;
    window.location.href = `mailto:eli@grantdeskhq.com?subject=${encodeURIComponent("GrantDesk Founding Pilot")}&body=${encodeURIComponent(body)}`;
  };
  ...
  return (
    <div className="site-shell py-14 lg:py-20">
      ...
      <div className="mt-8 grid gap-12 lg:grid-cols-[.9fr_1.1fr]">
        <section>
          <p className="eyebrow">One focused validation offer</p>
          ...
          <p className="mt-5 max-w-xl leading-7 text-slate-600">Evaluate the draft-and-evidence workflow against completed, historical reporting work with direct founder support.</p>
        ...
        <section className="panel shadow-panel">
          <p className="eyebrow">Discuss fit with the founder</p>
          <h2 className="text-2xl font-semibold text-navy-900">Founding pilot enquiry</h2>
          <p className="mt-3 text-sm leading-6 text-slate-600">Submitting opens your email application with a prefilled message to Eli Katz. This website does not transmit or store the form, and it does not accept file uploads.</p>
          <form className="mt-7 grid gap-5" onSubmit={submit}>
            ...
            <Field label="Current grant-reporting process" id="pilot-process"><textarea id="pilot-process" className="form-control min-h-28" required value={form.process} onChange={(event) => update("process", event.target.value)} placeholder="Briefly describe how your team currently assembles completed post-award reports." /></Field>
            <div className="border border-slate-200 bg-slate-50 p-4 text-xs leading-5 text-slate-600">Do not include or attach client files. Prospective participants will first discuss data-handling requirements with Eli.</div>
            <button type="submit" className="button button-primary button-large w-full"><Mail aria-hidden="true" /> Discuss the Founding Pilot <ArrowRight aria-hidden="true" /></button>
          </form>
        </section>
      </div>
    </div>
