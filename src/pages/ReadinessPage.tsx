  ...
  return <div className="readiness-page">
    <section className="readiness-hero">
      <div className="site-shell grid items-start gap-10 py-12 lg:grid-cols-[1fr_.72fr] lg:py-16">
        <div><div className="prototype-pill"><span aria-hidden="true" /> Free readiness audit · professional review required</div><p className="eyebrow mt-7">Know what the grant will require</p><h1>Find every reporting requirement before the deadline gets close.</h1><p>Upload the award agreement and GrantDeskHQ will identify reporting dates, financial schedules, program measures, required attachments, and missing evidence. Your team gets a source-linked plan instead of another manual checklist.</p><div className="mt-6 flex flex-wrap gap-3"><a className="button button-primary button-large" href="#readiness-form">Start the free audit <ArrowRight aria-hidden="true" /></a><button type="button" className="button button-secondary button-large" onClick={loadSyntheticSample} disabled={loadingSample}>{loadingSample ? <LoaderCircle className="animate-spin" aria-hidden="true" /> : <FileCheck2 aria-hidden="true" />}{loadingSample ? "Loading sample…" : "See a sample audit"}</button></div><p className="trust-line"><ShieldCheck aria-hidden="true" />AI-powered extraction · independent evidence check · human approval required</p></div>
    ...
    <section className="site-shell grid gap-8 py-10 lg:grid-cols-[.72fr_1.28fr] lg:py-14" id="readiness-form">
      <aside className="readiness-value"><LockKeyhole aria-hidden="true" /><h2>Start with the agreement, not another setup project.</h2><p>See what the funder actually requires and what your team still needs to collect before you buy anything.</p><ol><li><span>1</span>Add one award agreement</li><li><span>2</span>Optionally add the approved budget and separate instructions</li><li><span>3</span>Review the extracted plan and evidence coverage</li></ol></aside>
      <form className="compile-form" onSubmit={submit}>
        <div><p className="eyebrow">Free Grant Reporting Readiness Audit</p><h2 className="mt-2 text-2xl font-semibold text-navy-900">Turn one agreement into a reporting checklist</h2><p className="wizard-intro mt-2">For evaluation, use synthetic or appropriately redacted historical files. No accounting connection is required.</p></div>
