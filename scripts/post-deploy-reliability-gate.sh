...
rollback_if_allowed() {
  ...
  local lkg_token
  lkg_token="$(gcloud auth print-identity-token --project="${project}" --audiences="${lkg_origin%/}" --impersonate-service-account="${service_account}")"
...
token="$(gcloud auth print-identity-token --project="${project}" --audiences="${origin%/}" --impersonate-service-account="${service_account}")"
