...
export const STATUSES = new Set(["QUEUED", "RUNNING", "PASS", "PARTIAL", "BLOCKED", "FAILED", "SKIPPED"]);
...
export function queueSummary(queue) {
  ...
  return { total: queue.tasks.length, count, remaining: count.QUEUED + count.RUNNING, next: selectNextTask(queue)?.id || null };
}
...
export function writeStatus(queue, stateDir, statusFile = defaultStatusFile, extra = {}) {
  const summary = queueSummary(queue); const lines = ["GRANTDESKHQ CODEX QUEUE STATUS", "", "LAST UPDATED: " + now(), "TASKS TOTAL: " + summary.total, "QUEUED: " + summary.count.QUEUED, "RUNNING: " + summary.count.RUNNING, "PASSED: " + summary.count.PASS, "PARTIAL: " + summary.count.PARTIAL, "BLOCKED: " + summary.count.BLOCKED, "FAILED: " + summary.count.FAILED, "REMAINING: " + summary.remaining, "CURRENT TASK: " + (extra.currentTask || "none"), "LAST COMPLETED TASK: " + (extra.lastCompleted || "none"), "LAST CHECKPOINT: " + (extra.checkpoint || "none"), "HUMAN ACTIONS REQUIRED:"];
  const blockers = queue.tasks.filter((task) => task.status === "BLOCKED").map((task) => task.id + ": " + (task.blocker || "not recorded")); lines.push(...(blockers.length ? blockers : ["NONE"]));
  mkdirSync(stateDir, { recursive: true }); mkdirSync(dirname(statusFile), { recursive: true }); writeFileSync(statusFile, lines.join("\n") + "\n"); appendFileSync(join(stateDir, "queue.log"), now() + " status updated\n");
}

export function composePrompt(task, policy, root) {
  return ["You are the bounded GrantDeskHQ queue worker. Complete only the selected task below.", "Repository root: " + root, "", "PERSISTENT OPERATING POLICY", policy, "", "TASK", JSON.stringify(task, null, 2), "", "Completion contract:", "- Do not start another queue task.", "- Preserve existing validated work and never force-push, expose secrets, send outreach, alter production traffic, make purchases, or create live Stripe charges.", "- Run the listed acceptance checks. Do not weaken tests.", "- Final output must be JSON matching the supplied schema with status PASS, PARTIAL, BLOCKED, or FAILED, tests, artifacts, blocker, and result_summary."].join("\n");
}
...
export async function invokeCodex({ root, task, policy, schema, stateDir, maxRuntimeMs }) {
  ...
  const args = ["exec", "--strict-config", "--sandbox", "workspace-write", "--approve-for-me", "--json", "--output-schema", schema, "--output-last-message", finalFile, "-C", root, prompt];
  return await new Promise((resolvePromise) => {
    ...
    child.on("close", (code, signal) => { clearInterval(watchdog); let result = null; try { result = readJson(finalFile); } catch { result = null; } resolvePromise({ code: code ?? 1, signal, result, runDir, eventFile }); });
  });
}
...
export async function runQueue({ queuePath, policyPath, schemaPath, root, stateDir = defaultStateDir, statusFile = defaultStatusFile, maxRuntimeHours = 8, maxTasks = 20, dryRun = false }) {
  ...
  try {
    for (let processed = 0; processed < maxTasks && Date.now() - started < maxRuntimeHours * 3_600_000; processed += 1) {
      if (existsSync(stopPath(stateDir))) break; const task = selectNextTask(queue); if (!task) break;
      task.status = "RUNNING"; task.started_at = now(); task.attempt_count += 1; task.updated_at = now(); writeJson(queuePath, queue); writeStatus(queue, stateDir, statusFile, { currentTask: task.id, lastCompleted, checkpoint: "task started" });
      if (dryRun) { task.status = "PARTIAL"; task.blocker = "DRY_RUN: task selected but Codex was not invoked."; task.completed_at = now(); task.updated_at = now(); writeJson(queuePath, queue); lastCompleted = task.id; continue; }
      const execution = await invokeCodex({ root, task, policy, schema: schemaPath, stateDir, maxRuntimeMs: Math.max(60_000, maxRuntimeHours * 3_600_000 - (Date.now() - started)) });
      const verified = validateResult(execution.result, task, root); task.status = verified.status; task.blocker = verified.blocker; task.completed_at = now(); task.updated_at = now(); task.result_summary = execution.result?.result_summary || "No structured task summary returned."; task.commit_sha = execution.result?.commit_sha || null; task.run_dir = execution.runDir;
