...
if (command === "status") { const value = queue(); const usageFile = routingUsagePath(stateDir); const routingUsage = existsSync(usageFile) ? readJson(usageFile) : undefined; writeStatus(value, stateDir, statusFile, { routingUsage, budgetPolicy: loadBudgetPolicy(budgetPolicyPath) }); console.log(JSON.stringify(queueSummary(value), null, 2)); }
